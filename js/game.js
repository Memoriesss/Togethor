...
export class GameManager {
  ...
  startGame() {
    ...
    this.sceneManager.startAnimation(() => {
      ...
      if (this.playerTrain && !this.playerTrain.isFinished) {
        ...
        const speedFactor = this.playerTrain.currentSpeed / 40;
        const baseZ = 5;
        const speedBonus = (speedFactor - 1) * 3;
        const playerZ = baseZ - speedBonus;
        
        this.playerTrain.getObject().position.x = playerTrackPos.x;
        this.playerTrain.getObject().position.y = playerTrackPos.y + this.playerTrain.shakeOffset;
        this.playerTrain.getObject().position.z = playerZ;
        ...
        if (this.isDriving) {
          ...
          const speedExcess = this.playerTrain.currentSpeed - 40;
          const decayAmount = delta * 0.005 * speedExcess * speedExcess;
          if (decayAmount > 0) {
            this.playerTrain.decreaseSpeed(decayAmount);
          }
        } else {
          if (this.playerTrain.currentSpeed > 30) {
            this.playerTrain.decreaseSpeed(delta * 0.3);
          } else if (this.playerTrain.currentSpeed < 30) {
            this.playerTrain.setSpeed(30);
          }
        }
      }
      ...
      this.aiTrains.forEach((aiTrain) => {
        ...
        const aiSpeedFactor = aiTrain.currentSpeed / 40;
        const aiZ = relativeZ + (1 - aiSpeedFactor) * 2;
        
        aiTrain.getObject().position.x = aiTrackPos.x;
        aiTrain.getObject().position.y = aiTrackPos.y + aiTrain.shakeOffset;
        aiTrain.getObject().position.z = aiZ;
  ...
  async startAutoListening() {
    ...
    this.speechRecognizer.onInterimResult = (text) => {
      this.ui.updateSpeechInput(text);
    };
  ...
  async keepListening() {
    ...
    try {
      ...
      this.ui.updateSpeechInput(result.result);
      
      if (result.isMatch) {
        this.isListeningActive = false;
        await this.handleCorrect();
      } else if (this.isListeningActive && this.isPlaying) {
  ...
  checkCharInText(text, targetChar) {
    ...
    const pinyins = pinyinMap[targetChar] || [];
    const lowerText = text.toLowerCase();
    return pinyins.some(p => lowerText.includes(p.toLowerCase()));
  }
  ...
  async handleCorrect() {
    ...
    this.playerTrain.increaseSpeed(30);
