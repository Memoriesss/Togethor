...
export class Train {
  ...
  createCars(mainColor) {
    ...
    for (let i = 0; i < this.carCount; i++) {
      ...
      carGroup.position.z = startZ + i * (carLength + actualGap);
      carGroup.userData.carIndex = i;
      carGroup.userData.originalZ = carGroup.position.z;
      ...
      leftWindow.position.set(-1.31, 1.2, 0);
      carGroup.add(leftWindow);

      const rightWindow = new THREE.Mesh(windowGeometry, glassMaterial);
      rightWindow.position.set(1.31, 1.2, 0);
      ...
      frontWindow.position.set(0, 1.2, carLength / 2);
  ...
  createSmokeSystem(parentGroup) {
    ...
    const smokeGeo = new THREE.SphereGeometry(0.4, 10, 10);
    ...
    for (let i = 0; i < 30; i++) {
      ...
      smoke.scale.setScalar(0.8 + Math.random() * 0.8);
      smoke.userData = {
  ...
  update(delta) {
    ...
    const speedOver55 = this.isPlayer && this.currentSpeed > 55;
    const heavySmokeFactor = speedOver55 ? Math.min((this.currentSpeed - 55) / 8 + 1.5, 5) : 1;
    
    const shakeIntensity = 0.05 * speedFactor;
    this.shakeOffset = Math.sin(this.shakeTime * 15 * speedFactor) * shakeIntensity;
    this.group.position.y = this.shakeOffset;
    this.group.rotation.z = Math.sin(this.shakeTime * 12 * speedFactor) * shakeIntensity * 0.3;
    
    this.smokeParticles.forEach((smoke, index) => {
      const userData = smoke.userData;
      const driftZ = userData.driftZ * heavySmokeFactor;
      smoke.position.y = userData.initialY + Math.sin(this.smokeTime * userData.speed + userData.phase) * 0.5 * heavySmokeFactor;
      smoke.position.x = userData.initialX + Math.sin(this.smokeTime * userData.speed * 2 + userData.phase) * 0.4 * heavySmokeFactor;
      smoke.position.z = userData.initialZ + Math.cos(this.smokeTime * userData.speed + userData.phase) * 0.5 * heavySmokeFactor + this.smokeTime * driftZ;
      
      if (speedOver55) {
        smoke.material.color.setHex(0xFFFFFF);
        smoke.material.opacity = (0.6 + 0.4 * Math.sin(this.smokeTime * userData.speed + userData.phase)) * heavySmokeFactor;
        smoke.scale.setScalar((1.2 + 1.0 * Math.sin(this.smokeTime * userData.speed + userData.phase) + 1.0) * heavySmokeFactor);
      } else {
        smoke.material.color.setHex(0x666666);
        smoke.scale.setScalar((0.6 + 0.5 * Math.sin(this.smokeTime * userData.speed + userData.phase) + 0.4) * speedFactor);
