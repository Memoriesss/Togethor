  ...
  updateSceneObjects(moveDistance) {
    this.trackSegments.forEach(segment => {
      ...
      if (segment.position.z > 60) {
        ...
        const normalizedZ = ((segment.position.z % 1000) + 1000) % 1000;
        const curveIndex = Math.floor(normalizedZ / 2) % this.trackCurve.length;
        const curveData = this.trackCurve[curveIndex] || { x: 0, y: 0 };
    ...
    this.decorations.forEach(decoration => {
      ...
      if (decoration.position.z > 60) {
        ...
        const normalizedZ = ((decoration.position.z % 1000) + 1000) % 1000;
        const curveIndex = Math.floor(normalizedZ / 2) % this.trackCurve.length;
        const curveData = this.trackCurve[curveIndex] || { x: 0, y: 0 };
        
        const baseDistance = Math.abs(decoration.userData.baseX);
        const sign = decoration.userData.baseX >= 0 ? 1 : -1;
        decoration.position.x = curveData.x + sign * baseDistance;
